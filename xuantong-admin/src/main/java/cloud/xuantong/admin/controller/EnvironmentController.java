...
public class EnvironmentController {
    ...
    @Get
    @Mapping("/{code}")
    public Result<Environment> getEnvironment(@Path String code) {
    ...
    @Post
    @Mapping
    public Result<String> saveEnvironment(@Body Environment env) {
    ...
    @Put
    @Mapping("/{code}")
    public Result<String> updateEnvironment(@Path String code, @Body Environment env) {
        if (!code.equals(env.getCode())) {
            return Result.failure("环境编码不匹配");
        }
        boolean success = environmentService.updateEnvironment(env);
        return success ? Result.succeed("更新成功") : Result.failure("更新失败");
    }

    @Put
    @Mapping("/default/{code}")
    public Result<String> setDefaultEnvironment(@Path String code) {
    ...
    @Delete
    @Mapping("/{code}")
    public Result<String> deleteEnvironment(@Path String code) {
        boolean success = environmentService.deleteEnvironment(code);
