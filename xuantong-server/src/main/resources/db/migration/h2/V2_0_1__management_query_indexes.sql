...
CREATE INDEX IF NOT EXISTS idx_config_resource_list
    ON config_resource (namespace_id, group_name, lifecycle_status, data_id);

CREATE INDEX IF NOT EXISTS idx_config_rollout_history
    ON config_rollout (config_id, created_at, id);

CREATE INDEX IF NOT EXISTS idx_service_definition_list
...
CREATE INDEX IF NOT EXISTS idx_audit_type_history
    ON audit_log (resource_type, id);

CREATE INDEX IF NOT EXISTS idx_client_access_token_list
    ON client_access_token (is_active, id);

CREATE INDEX IF NOT EXISTS idx_user_list
    ON `user` (is_active, role, username);

CREATE INDEX IF NOT EXISTS idx_user_role_list
