...
CREATE TABLE IF NOT EXISTS config_item (
    ...
    value_type VARCHAR(10) DEFAULT 'STRING' COMMENT '值类型：STRING/NUMBER/BOOLEAN/JSON',
    CONSTRAINT uk_key_env_project UNIQUE (`key`, project, environment),
...
SELECT 'demo.nimbus.aaa', 'spring-boot4.0.6', 'aaa', 'dev', 'demo', 109, 'STRING', 0, 'admin'
WHERE NOT EXISTS (SELECT 1 FROM config_item WHERE `key` = 'demo.nimbus.aaa' AND environment = 'dev' AND project = 'demo');

INSERT INTO config_item (`key`, `value`, description, environment, project, version, value_type, is_encrypted, created_by)
SELECT 'demo.aaa.yml', 'solon.cloud.xxx:\n  server: \"localhost:8849,localhost:8847\" \n  namespace: \"3887EBC8-CD24-4BF7-BACF-58643397C138\" #nacos\n  contextPath: \"nacosx\"\n  username: \"aaa\"\n  password: \"bbb\"', '测试load', 'dev', 'demo', 3, 'STRING', 0, 'admin'
WHERE NOT EXISTS (SELECT 1 FROM config_item WHERE `key` = 'demo.aaa.yml' AND environment = 'dev' AND project = 'demo');

INSERT INTO config_item (`key`, `value`, description, environment, project, version, value_type, is_encrypted, created_by)
SELECT 'demo.list', '[{"name":"德莱厄斯","age":18},{"name":"锐雯","age":18}]', '演示JSON数组配置', 'dev', 'demo', 1, 'STRING', 0, 'admin'
WHERE NOT EXISTS (SELECT 1 FROM config_item WHERE `key` = 'demo.list' AND environment = 'dev' AND project = 'demo');

INSERT INTO config_item (`key`, `value`, description, environment, project, version, value_type, is_encrypted, created_by)
SELECT 'demo.map', '{"MALE":[{"name":"德莱厄斯","age":18}],"FEMALE":[{"name":"锐雯","age":18}]}', '演示JSON对象配置', 'dev', 'demo', 1, 'STRING', 0, 'admin'
